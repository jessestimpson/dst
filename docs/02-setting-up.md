...
There are 4 pieces of machinery. Once they're configured you rarely think about
3 of them.
...
The fourth is `dst_transform`, a parse transform you apply to the modules that
make up the system under test. It rewrites their timer, clock and spawn calls
to point at `dst_time` and `dst_sched` instead of `erlang`. It changes nothing
else about them.
...
The transform is opt-in per module, and guarded so it can't reach production:
...
with the simulation build defining `DST`. In Mix that's:
...
2 of those deserve more explanation.
...
`adopted_late` counts processes that ran before the scheduler owned them. A run
with a high count is a run whose interleaving is partly decided by wall clock,
and its seed won't reproduce.
