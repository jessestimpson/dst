defmodule DstFixtureTest do
  ...
  setup do
    dir = Path.join(System.tmp_dir!(), "dst_fixture_#{System.unique_integer([:positive])}")
    File.mkdir_p!(dir)
    on_exit(fn -> File.rm_rf!(dir) end)
    {:ok, path: Path.join(dir, "atomicity.dst")}
  end
  ...
  defp repro do
    seed =
      Enum.find(1..60, fn s ->
        match?({:violation, _}, :dst_run.run(@harness, Map.put(@buggy, :seed, s)).outcome)
      end)
    ...
    %{trace: trace, verified: true} =
      :dst_shrink.shrink(@harness, :dst_run.run(@harness, opts).trace, opts)
  ...
  describe "saving" do
    test "writes a fixture that carries its harness and options", %{path: path} do
      ...
      assert {:ok, {:violation, %{property: :atomicity}}} =
               :dst_run.save_fixture(path, @harness, trace, opts)

      fixture = :dst_run.load_fixture(path)
    ...
    test "refuses a trace that does not replay", %{path: path} do
      ...
      assert {:error, {:did_not_replay, {:diverged, _, _}}} =
               :dst_run.save_fixture(path, @harness, Enum.reverse(trace), opts)
  ...
  describe "replaying" do
    test "a test names a file and nothing else", %{path: path} do
      {trace, opts} = repro()
      {:ok, _} = :dst_run.save_fixture(path, @harness, trace, opts)

      assert %{outcome: {:violation, %{property: :atomicity}}} =
               :dst_run.replay_fixture(path)

      assert :dst_run.check_fixture(path) == :ok
    end
    ...
    test "the trace carries the repro, not the seed", %{path: path} do
      ...
      {trace, opts} = repro()
      {:ok, expected} = :dst_run.save_fixture(path, @harness, trace, opts)

      elsewhere = Map.put(opts, :seed, opts.seed + 9_999)

      assert %{outcome: ^expected} = :dst_run.replay(@harness, trace, elsewhere)
    end
    ...
    test "dropping the config it was saved with breaks it", %{path: path} do
      ...
      %{outcome: outcome} = :dst_run.replay(@harness, trace, Map.delete(opts, :config))
  ...
  describe "rejecting a stale file" do
    test "an unreadable term is named, not badmatched", %{path: path} do
      ...
      assert_raise ErlangError, fn -> :dst_run.load_fixture(path) end
    end
  end
end
