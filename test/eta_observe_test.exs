defmodule DstObserveTest do
  @moduledoc """
  `eta_observe` — reading a process's state without asking it.
  ...
  describe "{Record, Fields}" do
    test "publishes the named fields, and only those" do
      {:ok, pid} = :eta_observe_sut.start_link()

      assert :eta_observe.read(pid) == %{epoch: 0, leader: :undefined}

      :eta_observe_sut.bump(pid)
      :eta_observe_sut.get(pid)
    ...
    test "a bare field list is refused, rather than assuming #state{}" do
      ...
      assert {:error, _, _} = compile("-eta_observe([epoch]).")
    end

    test "an unknown field is a compile error, not a wrong element/2 offset" do
      assert {:error, _, _} = compile("-eta_observe({st, [nope]}).")
    end
  end

  describe "all" do
    test "publishes the whole state, even when it is not a record" do
      {:ok, pid} = :eta_observe_all_sut.start_link()

      assert :eta_observe.read(pid) == %{epoch: 0}

      :eta_observe_all_sut.bump(pid)
      :eta_observe_all_sut.get(pid)
      assert :eta_observe.read(pid) == %{epoch: 1}
  ...
  describe "reading" do
    ...
    test "a process that never published, and a dead one, both read undefined" do
      assert :eta_observe.read(self()) == :undefined
      assert :eta_observe.read(:no_such_registered_name) == :undefined

      pid = spawn(fn -> :ok end)
      Process.sleep(10)
      assert :eta_observe.read(pid) == :undefined
    end
  end

  # Compiles a throwaway module carrying `attr`, so a rejected `-eta_observe`
  # declaration can be asserted on rather than described.
  defp compile(attr) do
    name = :"eta_observe_probe_#{System.unique_integer([:positive])}"
