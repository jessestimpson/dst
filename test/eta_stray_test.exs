defmodule DstStrayTest do
  ...
  `eta_stray` reproduces that shape in eighty lines. See its moduledoc for the
  mechanism.
  ...
  **This was a real defect and it is fixed.** The driver now advances only to a
  ...
  `eta_time:advance_to_next/1`. These tests were written against the broken
  ...
  # about the system.
  @opts %{max_ops: 12, max_steps: 20_000, settle_steps: 300, preload: [:eta]}

  @deadline :eta_stray.deadline()

  defp run(seed, stray) do
    :eta_run.run(:eta_stray_harness, Map.merge(@opts, %{seed: seed, config: %{stray: stray}}))
  end
  ...
  for mode <- [:dead, :unowned] do
    describe "a #{mode} process's timer" do
      ...
      test "is reported by audit/1" do
        ...
        assert {:suspect, suspect} = :eta_run.audit(strayed)
