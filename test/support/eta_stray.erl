...
%% virtual wheel belonging to a process `eta_sched` does not own.
%%
...
%% under a single `receive ... after ?GATHER_TIMEOUT + 500`. `eta_transform` puts
...
%% made nothing runnable. `eta_run:audit/1` reported `ok` on both: nothing was
...
%% code:
%%
%% `dead` — the owner is gone. `eta_transform` puts `eta_time:disarm_after/2` at
...
%% adopt it. `eta_sched:spawn/1` falls back to a plain `erlang:spawn/1` when no
%% scheduler is active, and `eta_harness:init/2` runs before there is one. So
...
%%
%% Rewritten by `eta_transform` into a receive whose timeout is an ordinary
%% message clause, armed through `eta_time:arm_after/2`. The arming is what puts a
...
%% is `eta_sched:spawn/1`, which is a plain spawn whenever no scheduler is
