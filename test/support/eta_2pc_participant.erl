...
%% while every one of these processes is suspended. See `eta_harness` on why an
...
init({Tab, Index}) ->
    %% Names this process for `eta_log`. One call, at startup, and every event it
    %% records afterwards is attributed without any API having to carry a label.
    ok = ?ETA_LABEL({participant, Index}),
...
handle_cast({prepare, TxId, Coordinator}, St = #{tab := Tab, index := Index}) ->
    case plan(Tab, TxId, Index) of
        stall ->
            ...
            _ = ?ETA_LOG({stalling, TxId}),
            {noreply, St};
        Vote ->
            record(Tab, TxId, Index, {prepared, Vote}),
            _ = ?ETA_LOG({voted, TxId, Vote}),
...
handle_cast({decision, TxId, Decision}, St = #{tab := Tab, index := Index}) ->
    ...
    _ = ?ETA_LOG({decided, TxId, Decision, Final}),
    {noreply, St}.
