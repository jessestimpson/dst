-module(eta_2pc_coordinator).
-behaviour(gen_server).

%% A two-phase commit coordinator. The other half of `eta_2pc`.
%%
...
%% transform and the `?ETA_LOG` macros, and under a release build it brings
...
decide(TxId, Decision, St = #{tab := Tab, participants := Ps, pending := Pending}) ->
    ...
    _ = ?ETA_LOG({decided, TxId, Decision}),
