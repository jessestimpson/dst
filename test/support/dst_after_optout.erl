-module(dst_after_optout).

%% The escape hatch, and the only thing that tests it.
%%
%% `dst_transform` virtualises `receive ... after` by default, because a module
%% that quietly wakes on the wall clock under simulation is exactly the silent
%% failure the framework exists to remove. `-dst_after(false)` turns that pass
