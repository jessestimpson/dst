defmodule DstTimeTest do
  @moduledoc """
  Phase 1 exit criteria for `:dst_time` and `:dst_transform`
  (see docs/design/dst_framework_design.md).
  ...
  defp new_trace, do: :ets.new(:dst_time_trace, [:public, :ordered_set])
  ...
  describe "dst_transform" do
    test "a transformed module's timers and clock reads are virtualised" do
      ...
      assert @time.pending() == 1, "the SUT's send_after did not reach dst_time"
      assert events(trace) == []
  ...
  describe "speed" do
    test "a run spanning simulated hours completes in milliseconds" do
      ...
      #
      # Driven through dst_sched rather than by calling advance_to_next/0 in a
  ...
  describe "event loop with dst_sched" do
