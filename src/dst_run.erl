...
The system under test supplies six callbacks; see `dst_harness`.
...
-type entry() :: {op, dst_harness:op()} | {step, dst_sched:id()} | {clock, integer()}.
-type outcome() ::
    ok
    | {violation, dst_harness:violation()}
    | {error, term()}.
-type result() :: #{
    outcome := outcome(),
    seed := integer(),
...
%% slower is a `check/1` that called into a suspended process — see `dst_harness`.
...
loading `kernel`, `stdlib` and `dst` rather than waiting to be asked, and the
...
```erlang
ok = dst_run:audit(dst_run:run(Mod, Opts)).
```
""".
-endif.
...
- `sched.adopted_late` — processes that ran before the scheduler owned them.
  Fix by spawning through `dst_run:spawn_op/1` and `dst_sched:spawn/1`.
...
replayed without it does not reproduce, and `dst_shrink` reports "nothing to
...
Take the trace from a `dst_shrink:shrink/3` result whose `verified` is `true`,
which is `dst_shrink`'s own version of this check.
...
load_fixture(Path) ->
    ...
    case binary_to_term(Bin) of
        ...
        #{version := V} -> error({dst_run, {unsupported_fixture_version, V, Path}});
        _ -> error({dst_run, {not_a_fixture, Path}})
    end.
...
drive(Mod, Opts, Source) ->
    ...
    try
        ...
        Sched0 = dst_sched:new(#{seed => Seed}),
        Sched = dst_sched:register(Sched0, Mod:processes(Sut)),
...
%%
%% Deliberately not automatic beyond the defaults. `dst_run` is handed a module,
...
preload(Apps) when is_list(Apps) ->
    ...
    %% would think to name them; `dst` because a transformed module's rewritten
    %% calls land in `dst_time` from inside a scheduled process.
    lists:foreach(fun preload_app/1, [kernel, stdlib, dst | Apps]).

preload_app(App) ->
    case application:load(App) of
        ok -> ok;
        {error, {already_loaded, App}} -> ok;
        {error, Reason} -> error({dst_run, {preload_failed, App, Reason}})
    end,
...
%% making the invariant vacuous — which is why `dst_harness` says what it says.
safe_check(Mod, Sut) ->
